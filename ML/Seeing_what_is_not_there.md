...
这种周边环境和目标分离的好处在于，我们不需要在训练集中标记不正常（缺失、不符合环境预期而出现的物体）的目标物体，
独立周边环境模型可以由典型目标物体训练（学习典型目标物体所处的位置）得到，而后用于寻找不正常目标物体。
...
1. 负样本：每张图片中心区域采用类似的遮罩层随机裁剪。被覆盖的区域与ground truth标记物体重合不大于Jaccard（IoU）系数0.2。
...
在每个位置应用固定大小（即滑动窗口，本文实现中使用224*224）中心遮罩的图像块输入基础网络。中心遮罩的图像块的大小参照训练集根据经验决定。
...
较低的loss表示网络对于遮罩和非遮罩图片（一般图片）的输出更接近。

因此，SFC表现与预期一致：

1. SFC习得非显示遮罩，因此对中心区域变化不敏感。
1. SFC在分类任务中的有用特征主要由非遮罩区域获得。
...
这里的"正样本"是缺失的路缘坡道。由于Faster RCNN有能力识别周边环境，并且它是end-to-end approach。
因此我们认为Faster RCNN是一个较强的baseline。
...
1. 与SFC相比，Faster RCNN检测器的recall明显偏低。若有更多的训练数据，Faster RCNN可能表现更好。
...
模型基于卷积神经网络，提供了学习非显式遮罩的方法，以便于网络忽略目标物体本身而专注于周边环境。
